function flush!(A::ExtendableSparseMatrixParallel; do_dense = false, keep_zeros = true)
    ...
    return A.lnkmatrices = [SuperSparseMatrixLNK{matrixvaluetype(A), matrixindextype(A)}(A.n, A.nnts[tid]) for tid in 1:A.nt]
...
function sparse_flush!(A::ExtendableSparseMatrixParallel; keep_zeros = true)
    ...
    #dropzeros!(
    return plus_remap(A.lnkmatrices, A.cscmatrix, A.globalindices; keep_zeros)
    #)
...
function dense_flush_keepzeros!(
    ...
    nnz = sum([As[i].nnz for i in 1:nt]) #you could also subtract the diagonal entries from shared columns, since those are definitely double
