...
function ExtendableSparseMatrixParallel{Tv, Ti}(mat_cell_node, nc, nn, nt, depth; block_struct = true) where {Tv, Ti <: Integer}
    ...
    lnk = [SuperSparseMatrixLNK{Tv, Ti}(nn, nnts[tid]) for tid in 1:nt]
    return ExtendableSparseMatrixParallel{Tv, Ti}(csc, lnk, nnts, s, onr, cfp, gi, ni, rni, starts, cellparts, nt, depth, phash(csc), csc.n, csc.m)
end
...
function addtoentry!(A::ExtendableSparseMatrixParallel{Tv, Ti}, i, j, tid, v; known_that_unknown = false) where {Tv, Ti <: Integer}
    ...
    return if updatentryCSC2!(A.cscmatrix, i, j, v)
    else
        A.lnkmatrices[tid][i, A.sortednodesperthread[tid, j]] += v
    end
end
...
function updateindex!(
        ext::ExtendableSparseMatrixParallel{Tv, Ti},
        op,
        v,
        i,
        j
...
function Base.getindex(
    ...
    level, tid = last_nz(ext.old_noderegions[:, ext.rev_new_indices[j]])
    return ext.lnkmatrices[tid][i, ext.sortednodesperthread[tid, j]]
...
function reset!(A::ExtendableSparseMatrixParallel{Tv, Ti}) where {Tv, Ti <: Integer}
    ...
    return A.lnkmatrices = [SuperSparseMatrixLNK{Tv, Ti}(A.n, A.nnts[tid]) for tid in 1:A.nt]
end
...
function nnz_noflush(ext::ExtendableSparseMatrixParallel)
    return nnz(ext.cscmatrix), sum([ext.lnkmatrices[i].nnz for i in 1:ext.nt])
end
...
function entryexists2(CSC, i, j) #find out if CSC already has an nonzero entry at i,j
    ...
    return i in view(CSC.rowval, CSC.colptr[j]:(CSC.colptr[j + 1] - 1))
end
...
function matvec!(y::AbstractVector{Tv}, A::ExtendableSparseMatrixParallel{Tv, Ti}, x::AbstractVector{Tv}) where {Tv, Ti <: Integer}
    ...
    for level in 1:depth
        @threads for tid::Int64 in 1:nt
            for col::Int64 in A.start[(level - 1) * nt + tid]:(A.start[(level - 1) * nt + tid + 1] - 1)
                for row::Int64 in colptr[col]:(colptr[col + 1] - 1) # in nzrange(A, col)
    ...
    @threads for tid in 1:1
        for col::Int64 in A.start[depth * nt + 1]:(A.start[depth * nt + 2] - 1)
            for row::Int64 in colptr[col]:(colptr[col + 1] - 1) #nzrange(A, col)
                y[rv[row]] += nzv[row] * x[col]
            end
        end
    end
