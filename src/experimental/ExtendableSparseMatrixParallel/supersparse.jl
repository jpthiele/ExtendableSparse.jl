...
function SparseArrays.SparseMatrixCSC(A::SuperSparseMatrixLNK{Tv, Ti})::SparseArrays.SparseMatrixCSC where {Tv, Ti <: Integer}
    return SparseArrays.SparseMatrixCSC(SparseMatrixLNK{Tv, Ti}(A.m, A.n, A.nnz, A.nentries, A.colptr, A.rowval, A.nzval))

end

function SuperSparseMatrixLNK{Tv, Ti}(m, n) where {Tv, Ti <: Integer}
    return SuperSparseMatrixLNK{Tv, Ti}(m, n, 0, n, zeros(Ti, n), zeros(Ti, n), zeros(Tv, n), zeros(Ti, n), 0)
end
...
function plus_remap(lnks::Vector{SuperSparseMatrixLNK{Tv, Ti}}, csc::SparseArrays.SparseMatrixCSC, gi::Vector{Vector{Ti}}; keep_zeros = true) where {Tv, Ti <: Integer}
    ...
    lnkscols = vcat([lnks[i].collnk[1:lnks[i].colctr] for i in 1:nt]...)
    supersparsecolumns = vcat([gi[i][lnks[i].collnk[1:lnks[i].colctr]] for i in 1:nt]...)
    num_cols = sum([lnks[i].colctr for i in 1:nt])
    tids = Vector{Ti}(undef, num_cols)
    ctr = 0
    for i in 1:nt
        for j in 1:lnks[i].colctr
            ctr += 1
            tids[ctr] = i
        end
    end
    ...
    coll = sum([lnks[i].nnz for i in 1:nt])
    ...
    col = [ColEntry{Tv, Ti}(0, zero(Tv)) for i in 1:coll]
    numshifts = 0
    ...
    ctr_last = 1
    gj_last = 0
    for J in 1:(length(sortedcolumns) - 1)
    ...
    return SparseArrays.SparseMatrixCSC(csc.m, csc.n, colptr, rowval, nzval)
...
function plus_remap(lnk::SuperSparseMatrixLNK{Tv, Ti}, csc::SparseArrays.SparseMatrixCSC, gi::Vector{Ti}) where {Tv, Ti <: Integer}
    ...
    col = [ColEntry{Tv, Ti}(0, zero(Tv)) for i in 1:csc.m]
...
function form(x)
    return [minimum(x), mean(x), maximum(x)]
end
