...
function testgrid(N; dim = 3)
    ...
    return simplexgrid((X for i in 1:dim)...)
end

function coordmatrix!(C, coord, cellnodes, k)
    spacedim = size(coord, 1)
    celldim = size(cellnodes, 1)
    return @inbounds for jj in 1:celldim
        C[1, jj] = 1
        @inbounds for ii in 1:spacedim
...
function stiffness!(S, dim, G)
    @inbounds for il in 1:(dim + 1)
        S[il, il] = scalpro(G, dim, il, il)
        @inbounds for jl in (il + 1):(dim + 1)
...
function testassemble_parallel!(A_h, grid)
    ...
    factdim::Float64 = factorial(dim)
    SS = [zeros(lnodes, lnodes) for i in 1:npart] # local stiffness matrix
    CC = [zeros(lnodes, lnodes) for i in 1:npart] # local coordinate matrix
    GG = [zeros(lnodes, lnodes) for i in 1:npart] # shape function gradients
